...
pub(crate) fn reproduce(population: &mut Population, pool: Rc<RefCell<GenePool>>, config: Rc<ReproductionConfig>, evaluation_config: Rc<EvaluationConfig>) -> Vec<Organism> {
  match config.global_strategy {
    GlobalReproductionStrategy::Probability => reproduce_probabilistic(population, pool, config.as_ref(), evaluation_config),
    GlobalReproductionStrategy::Fair => reproduce_fair(population, pool, config.as_ref(), evaluation_config)
  }
}

fn reproduce_fair(population: &mut Population, pool: Rc<RefCell<GenePool>>, config: &ReproductionConfig, evaluation_config: Rc<EvaluationConfig>) -> Vec<Organism> {
  let mut new_population: Vec<Organism> = Vec::with_capacity(config.organism_count);

  let mut total_fitness: f64 = population.species.iter_mut().map(|s| s.adjusted_fitness()).sum();
  if total_fitness == 0.0 {
    total_fitness = 1.0;
  }

  for species in &mut population.species {
    let target_count = (species.adjusted_fitness() / total_fitness * config.organism_count as f64).round() as usize;
    reproduce_species(species, target_count, Rc::clone(&pool), config, Rc::clone(&evaluation_config), &mut new_population)
  }

  new_population
}

fn reproduce_probabilistic(population: &mut Population, pool: Rc<RefCell<GenePool>>, config: &ReproductionConfig, evaluation_config: Rc<EvaluationConfig>) -> Vec<Organism> {
  todo!();
}

fn reproduce_species(species: &mut Species, target_count: usize, pool: Rc<RefCell<GenePool>>, config: &ReproductionConfig, evaluation_config: Rc<EvaluationConfig>, new_population: &mut Vec<Organism>) {
  ...
  let limit = max((species.organisms.len() as f64 * config.kill_ratio) as usize, species.organisms.len() - 1);
  ...
  if config.allow_elitism && species.organisms.len() >= config.elitism_limit {
    new_population.extend(species.organisms.iter().skip(species.organisms.len() - config.elitism_count).map(|x|(**x).clone()));
  }
  ...
  for _ in 0..(target_count - elitism_count - mutation_count) {
    ...
    let mut offspring = Organism::new(first_parent.genome.crossover(&second_parent.genome, &pool.borrow(), &config.crossover), Rc::clone(&pool), Rc::clone(&evaluation_config));
...
fn select_parent(species: &Species, strategy: &SpeciesReproductionStrategy, limit: usize) -> Rc<Organism> {
  match strategy {
    SpeciesReproductionStrategy::Random => Rc::clone(species.organisms[limit..].choose(&mut rand::thread_rng()).unwrap()),
...
fn mutate_organism(organism: &mut Organism, pool: &Rc<RefCell<GenePool>>, species_size: usize, config: &ReproductionConfig) {
  if species_size >= config.large_species_size {
    organism.genome.mutate(&mut pool.borrow_mut(), &config.large_intensity_config);
  } else {
    organism.genome.mutate(&mut pool.borrow_mut(), &config.large_intensity_config);
  }
}
