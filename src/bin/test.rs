...
use rusty_neat::{activation::Activation, gene_pool::GenePool, genome::{CrossoverConfig, CrossoverWeightStrategy, DistanceConfig, EvaluationConfig, Genome, MutationConfig, NewConnectionWeight}, population::{Population, PopulationConfig}, reproduction::{GlobalReproductionStrategy, ReproductionConfig, SpeciesReproductionStrategy}, serialize, species::{FitnessStrategy, ReprentativeSelection, SpeciesConfig}};
...
fn main() {
  ...
  let mut organism = population.evolve(|organisms| organisms.iter_mut().for_each(|organism| {
    let mut score = 0.0;
