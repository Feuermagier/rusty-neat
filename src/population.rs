...
impl Population {
  ...
  pub fn evolve<F: Fn(&mut [Organism]) -> ()> (&mut self, fitness_function: F) -> Organism {
    ...
    while generation <= max_generations && best_organism.fitness.unwrap() <= target_fitness {
      println!("Generation {}: {} organisms, {} species", generation, self.organisms.len(), self.species.len());
      // Neue Organismen erzeugen (durch Elitismus, Kreuzung und Mutationen)
      println!("Reproducing...");
      let mut new_organisms = reproduction::reproduce(self, Rc::clone(&self.pool), Rc::clone(&self.config.reproduction), Rc::clone(&self.config.evaluation));
      ...
      best_organism = Rc::clone(self.organisms.iter().max_by(|x, y| x.fitness.unwrap().partial_cmp(&y.fitness.unwrap()).unwrap()).unwrap());
      println!("=> Best fitness: {:.10} using {} nodes", best_organism.fitness.unwrap(), best_organism.genome.node_count());
  ...
  fn generate_initial_population<F: Fn(&mut [Organism]) -> ()> (&mut self, fitness_function: F) {
    let mut organisms: Vec<Organism> = Vec::with_capacity(self.config.reproduction.organism_count);

    for _ in 0..self.config.reproduction.organism_count {
      organisms.push(Organism::new(self.pool.borrow_mut().new_genome(&self.config.initial_organism_weight), Rc::clone(&self.pool), Rc::clone(&self.config.evaluation)));
    }
  ...
  fn speciate(&mut self) {
    ...
    for species in &self.species {
      new_species.push(Species::new(species.select_new_representative(), Rc::clone(&self.pool), Rc::clone(&self.config.species)));
    }
    ...
    for organism in &self.organisms {
      ...
      if !found_species {
        let mut species = Species::new(Rc::clone(&organism), Rc::clone(&self.pool), Rc::clone(&self.config.species));
        species.add_organism(Rc::clone(&organism));
