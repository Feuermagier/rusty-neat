...
impl From<(&PrintableSpecies, &PrintableGenePool)> for Species {
    fn from((species, pool): (&PrintableSpecies, &PrintableGenePool)) -> Self {
        let mut organisms: Vector<Arc<Organism>> = species.organisms.iter().map(|o| Arc::new((o, pool).into())).collect();
        ...
        Self {
            ...
            organisms,
            best_organism,
            average_fitness: species.organisms.iter().filter(|o| o.fitness.is_some()).map(|o| o.fitness.unwrap()).sum::<f64>() / species.organisms.len() as f64,
