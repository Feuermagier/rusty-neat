...
use rusty_neat::{activation::Activation, gene_pool::{Connection, GenePool}, genome::{EvaluationConfig, Genome, NewConnectionWeight}};
...
fn evaluate_large_flat(criterion: &mut Criterion) {
  ...
  criterion.bench_function("evaluate large flat network", |b| b.iter(|| genome.evaluate(&input, &pool, &config)));
}

fn create_genome_from_large_pool(criterion: &mut Criterion) {
  let pool = create_large_flat_pool();
  criterion.bench_function("create genome from large pool", |b| b.iter(|| pool.new_genome(&NewConnectionWeight::FIXED(1.0))));
}
...
fn add_connection_to_minimal_genome(criterion: &mut Criterion) {
  ...
  criterion.bench_function("add connection to minimal genome", |b| b.iter(|| genome.add_new_connection(connection, &NewConnectionWeight::FIXED(1.0))));
}
